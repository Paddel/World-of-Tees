...
template <typename T>
inline T vclamp(T val, T min, T max)
{
	if(val < min)
		return min;
	if(val > max)
...
inline vector2_base<T> closest_point_on_line(vector2_base<T> line_point0, vector2_base<T> line_point1, vector2_base<T> target_point)
{
	...
	T t = dot(v, c)/d;
	return mix(line_point0, line_point1, vclamp(t, (T)0, (T)1));
	/*
	if (t < 0) t = 0;
...
inline double PerpDot(const vec2 a,const vec2 b)                    { return (a.y*b.x) - (a.x*b.y); }

template<typename T>
inline bool lines_crossed(vector2_base<T> aPos1, vector2_base<T> aPos2, vector2_base<T> bPos1, vector2_base<T> bPos2, vector2_base<T> *pOutPos)
{
	vec2 a(aPos2-aPos1);
    vec2 b(bPos2-bPos1);
