...
struct CExtendInputArg
{
	int m_Type;
	char m_aName[32];
	CExtendInputArg(int Type) : m_Type(Type) {}

	virtual void Reset() = 0;
};

struct CExtendInputInt : public CExtendInputArg
{
	int m_StartVal;
	int m_Val;
	CExtendInputInt(char *pName, int Val) : m_Val(Val), m_StartVal(Val), CExtendInputArg(EXTINPTYPE_INT)
	{ str_copy(m_aName, pName, sizeof(m_aName)); }

	virtual void Reset() { m_Val = m_StartVal; }
};

struct CExtendInputStr : public CExtendInputArg
{
	char m_aStartVal[EXPINP_STR_MAX_LEN];
	char m_aVal[EXPINP_STR_MAX_LEN];
	CExtendInputStr(char *pName, char *pVal) : CExtendInputArg(EXTINPTYPE_STR)
	{
		str_copy(m_aVal, pVal, sizeof(m_aVal));
		str_copy(m_aStartVal, pVal, sizeof(m_aStartVal));
		str_copy(m_aName, pName, sizeof(m_aName));
	}

	virtual void Reset() { str_copy(m_aVal, m_aStartVal, sizeof(m_aVal)); }
};

class CExtendInput
{
public:
	array <CExtendInputArg *> m_lpInputArgs;
	int m_Index;
	char m_aName[128];

	CExtendInput(int Index, char *pName) : m_Index(Index)
	{
		str_copy(m_aName, pName, sizeof(m_aName));
	}
};
...
public:
	...
	virtual int GetTileType() { return -1; }
...
public:
	...
	CExtendInput *m_pEditingExtInp;
	CExTile *m_pEditingExTile;

	void MakeGameGroup(CLayerGroup *pGroup);
	void MakeGameLayer(CLayer *pLayer);
	void MakeExLayer(CLayer *pLayer);
	...
	class CLayerExTiles *m_pExLayer;
...
public:
	...
	virtual int GetTileType() { return m_LayerType; }
...
class CLayerExTiles : public CLayerTiles
{
public:
	CLayerExTiles(int w, int h);
	~CLayerExTiles();
...
public:
	...
	array<CExtendInput *> m_lpExtendInput;

	void RegisterExtendInput(int Index, char *pName, char *pArgs);
	...
	CEditor() : m_TilesetPicker(16, 16)
	{
		...
		m_RandomAnimateColor = 0;
		m_RandomAnimatePosition = 0;
		m_RandomAnimateSync = 0;
		...
		ms_ExEntitiesTexture = 0;

		ms_pUiGotContext = 0;
		m_ExtendedDir = 0;

		m_UseExMapInfo = false;

		m_UiNumPopups = 0;
	}

	void CheckExTile(int x, int y);

	virtual void Init();
	virtual void UpdateAndRender();
	virtual bool HasUnsavedData() { return m_Map.m_Modified; }

	void RegisterExtendetTiles();
	...
	bool m_RandomAnimateColor;
	bool m_RandomAnimatePosition;
	bool m_RandomAnimateSync;

	int m_AnimatePointSet;

	static int ms_CheckerTexture;
	static int ms_BackgroundTexture;
	static int ms_CursorTexture;
	static int ms_EntitiesTexture;
	static int ms_ExEntitiesTexture;
	...
	float UiDoValueSelectorF(void *pID, CUIRect *pRect, const char *pLabel, float Current, float Min, float Max, float Step, float Scale, const char *pToolTip, bool Float = false);
	...
	static int PopupExtInp(CEditor *pEditor, CUIRect View);
	static int PopupChestContent(CEditor *pEditor, CUIRect View);
	static int PopupEnvelopeMenu(CEditor *pEditor, CUIRect View);
	...
	void EnvelopeGenerateCodePoint(IOHANDLE File, CEnvelope *pEnvelope, int Index);
	void EnvelopeGenerateCode(CEnvelope *pEnvelope);
};
