...
void CEditor::UiInvokePopupMenu(void *pID, int Flags, float x, float y, float Width, float Height, int (*pfnFunc)(CEditor *pEditor, CUIRect Rect), void *pExtra)
{
	...
	if(y + Height > UI()->Screen()->h)
		y -= Height;
	s_UiPopups[m_UiNumPopups].m_pId = pID;
	s_UiPopups[m_UiNumPopups].m_IsMenu = Flags;
	s_UiPopups[m_UiNumPopups].m_Rect.x = x;
	s_UiPopups[m_UiNumPopups].m_Rect.y = y;
	s_UiPopups[m_UiNumPopups].m_Rect.w = Width;
	s_UiPopups[m_UiNumPopups].m_Rect.h = Height;
	s_UiPopups[m_UiNumPopups].m_pfnFunc = pfnFunc;
	s_UiPopups[m_UiNumPopups].m_pExtra = pExtra;
	m_UiNumPopups++;
}

void CEditor::UiDoPopupMenu()
{
	for(int i = 0; i < m_UiNumPopups; i++)
	{
...
int CEditor::PopupGroup(CEditor *pEditor, CUIRect View)
{
	...
	if(!pEditor->m_Map.m_pExLayer)
	{
		// new quad layer
		View.HSplitBottom(5.0f, &View, &Button);
		View.HSplitBottom(12.0f, &View, &Button);
		static int s_NewTileLayerButton = 0;
		if(pEditor->DoButton_Editor(&s_NewTileLayerButton, "Create extented Layer", 0, &Button, 0, "Creates the extended layer"))
		{
			CLayer *l = new CLayerExTiles(50, 50);
			pEditor->m_Map.MakeExLayer(l);
			pEditor->m_Map.m_pGameGroup->m_lLayers.add(l);
			return 1;
		}
	}
...
int CEditor::PopupLayer(CEditor *pEditor, CUIRect View)
{
	...
	if(pEditor->m_Map.m_pGameLayer == pEditor->GetSelectedLayer(0) || pEditor->m_Map.m_pExLayer == pEditor->GetSelectedLayer(0)) // dont use Group and Detail from the selection if this is the game layer
	{
...
int CEditor::PopupQuad(CEditor *pEditor, CUIRect View)
{
	...
	{
		for (int i = 0; i < 4; i++)
			dbg_msg(0, "#%i: %i,%i", i, pQuad->m_aTexcoords[i].x, pQuad->m_aTexcoords[i].y);
		//return 1;
		int Top = pQuad->m_aPoints[0].y;
		int Left = pQuad->m_aPoints[0].x;
...
int CEditor::PopupMapInfo(CEditor *pEditor, CUIRect View)
{
	...
	if(pEditor->m_UseExMapInfo)
	{
		...
		if(pEditor->UI()->MouseInside(&Label))
		{
			...
			{
				pEditor->m_Map.m_MapInfo.m_MapType = clamp(pEditor->m_Map.m_MapInfo.m_MapType+1, 0, NUM_MAPTYPES-1);
		...
		pEditor->UI()->DoLabel(&Label, s_aMapTypeNames[pEditor->m_Map.m_MapInfo.m_MapType], 12.0f, -1, -1);
		...
		if(pEditor->UI()->MouseInside(&Label))
		{
			if(pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_UP))
			{
				pEditor->m_Map.m_MapInfo.m_Temperature = ClampTemp(pEditor->m_Map.m_MapInfo.m_Temperature + 1);
				pEditor->m_ZoomLevel += 20;
			}
			else if(pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_DOWN))
			{
		...
		str_format(aBuf, sizeof(aBuf), "Temperature: %i", pEditor->m_Map.m_MapInfo.m_Temperature);
		...
		if (pEditor->UI()->MouseInside(&Label))
		{
			if (pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_UP))
			{
				pEditor->m_Map.m_MapInfo.m_Moisture = ClampMoisture(pEditor->m_Map.m_MapInfo.m_Moisture + 1);
				pEditor->m_ZoomLevel += 20;
			}
			else if (pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_DOWN))
			{
		...
		str_format(aBuf, sizeof(aBuf), "Moisture: %i", pEditor->m_Map.m_MapInfo.m_Moisture);
		pEditor->UI()->DoLabel(&Label, aBuf, 12.0f, -1, -1);
		...
		int Biome = CalculateBiome(pEditor->m_Map.m_MapInfo.m_Temperature, pEditor->m_Map.m_MapInfo.m_Moisture);
		pEditor->UI()->DoLabel(&Label, s_aMapBiomNames[Biome], 12.0f, -1, -1);
		...
		if (pEditor->UI()->MouseInside(&Label))
		{
			if (pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_UP))
			{
				pEditor->m_Map.m_MapInfo.m_Eruption = ClampEruption(pEditor->m_Map.m_MapInfo.m_Eruption + 1);
				pEditor->m_ZoomLevel += 20;
			}
			else if (pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_DOWN))
			{
...
int CEditor::PopupExtInp(CEditor *pEditor, CUIRect View)
{
	char aArgStr[MAX_EXTENTED_STR];
	mem_zero(&aArgStr, sizeof(aArgStr));

	// remove layer button
	CUIRect Button;
	CExtendInput *pExtInp = pEditor->m_Map.m_pEditingExtInp;
	CExTile *pExTile = pEditor->m_Map.m_pEditingExTile;
	View.HSplitTop(4.0f, &View, &Button);
	//name
	static float s_Name = 0;
	pEditor->UI()->DoLabel(&Button, pExtInp->m_aName, 14.0f, -1, -1);

	float FontSize = 10.0f;
	int Size = pExtInp->m_lpInputArgs.size();
	static float *s_aVal = new float[Size];
	for(int i = 0; i < Size; i++)
	{
		s_aVal[i] = 0;
		Button.HSplitTop(5.0f+12.0f, &View, &Button);
		Button.HSplitTop(12.0f, &Button, &View);
		pEditor->UI()->DoLabel(&Button, pExtInp->m_lpInputArgs[i]->m_aName, FontSize, -1, -1);

		int w = pEditor->TextRender()->TextWidth(0, FontSize, pExtInp->m_lpInputArgs[i]->m_aName, -1) +4;
		Button.VSplitLeft( w, 0, &Button);
		if(pExtInp->m_lpInputArgs[i]->m_Type == EXTINPTYPE_STR)
		{
			char *pVal = ((CExtendInputStr *)pExtInp->m_lpInputArgs[i])->m_aVal;
			if(pEditor->DoEditBox(&s_aVal[i], &Button, pVal, EXPINP_STR_MAX_LEN, 10.0f, &s_aVal[i]))
				pEditor->m_Map.m_Modified = true; 
		}
		else if(pExtInp->m_lpInputArgs[i]->m_Type == EXTINPTYPE_INT)
		{
			int *pVal = &((CExtendInputInt *)pExtInp->m_lpInputArgs[i])->m_Val;
			char aBuf[8];
			str_format(aBuf, sizeof(aBuf), "%i", *pVal);
			if(pEditor->DoEditBox(&s_aVal[i], &Button, aBuf, sizeof(aBuf), 10.0f, &s_aVal[i]))
				pEditor->m_Map.m_Modified = true; 
	...
	if(str_comp(pExTile->m_ExArgs, aArgStr) != 0)
		str_copy(pExTile->m_ExArgs, aArgStr, sizeof(aArgStr));

	return 0;
}

static char s_aChestContent[512];
...
{
	CUIRect Button;
	static int s_SelectedChest = 0;
	static int s_LastSelectedChest = -1;
	char aBuf[64];
	float FontSize = 10.0f;
	...
	if(pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_UP))
	{
		s_SelectedChest = clamp(s_SelectedChest+1, 0, NumChests-1);
		pEditor->m_ZoomLevel += 20;
	}
	else if(pEditor->Input()->KeyPressed(KEY_MOUSE_WHEEL_DOWN))
	{
	...
	str_format(aBuf, sizeof(aBuf), "%i/%i", s_SelectedChest+1, NumChests);
	pEditor->UI()->DoLabel(&Button, aBuf, FontSize, -1, -1);

	Button.HSplitTop(18, NULL, &Button);

	str_format(aBuf, sizeof(aBuf), "ID: %i", s_SelectedChest);
	pEditor->UI()->DoLabel(&Button, aBuf, FontSize, -1, -1);

	Button.HSplitTop(11, NULL, &Button);
	pEditor->UI()->DoLabel(&Button, s_aChestContent, FontSize, -1, -1);
	...
	s_LastSelectedChest = s_SelectedChest;
	return 0;
}

int CEditor::PopupEnvelopeMenu(CEditor *pEditor, CUIRect View)
{
	CUIRect Button;
	CEnvelope *pNewEnv = 0;
	int Close = 0;

	if (pEditor->m_SelectedEnvelope < 0 || pEditor->m_SelectedEnvelope >= pEditor->m_Map.m_lEnvelopes.size())
		return 1;

	View.HSplitTop(4.0f, &Button, &View);
	View.HSplitTop(15.0f, &Button, &View);
	
	static int s_New4dButton = 0;
	if (pEditor->DoButton_Editor(&s_New4dButton, "Color+", 0, &Button, 0, "Creates a new color envelope"))
	{
		pEditor->m_Map.m_Modified = true;
		pNewEnv = pEditor->m_Map.NewEnvelope(4);
		Close = 1;
	}
	...
	if (pEditor->DoButton_Editor(&s_New2dButton, "Pos.+", 0, &Button, 0, "Creates a new pos envelope"))
	{
		pEditor->m_Map.m_Modified = true;
		pNewEnv = pEditor->m_Map.NewEnvelope(3);
		Close = 1;
	}
	...
	if (pEditor->DoButton_Editor(&s_DelButton, "Delete", 0, &Button, 0, "Delete this envelope"))
	{
		pEditor->m_Map.m_Modified = true;
		pEditor->m_Map.DeleteEnvelope(pEditor->m_SelectedEnvelope);
		if (pEditor->m_SelectedEnvelope >= pEditor->m_Map.m_lEnvelopes.size())
			pEditor->m_SelectedEnvelope = pEditor->m_Map.m_lEnvelopes.size() - 1;
	...
	static int s_CloneButton = 0;
	if (pEditor->DoButton_Editor(&s_CloneButton, "Clone", 0, &Button, 0, "Clone this envelope"))
	{
		pNewEnv = pEditor->m_Map.NewEnvelope(pEditor->m_Map.m_lEnvelopes[pEditor->m_SelectedEnvelope]->m_Channels);
		*pNewEnv = *pEditor->m_Map.m_lEnvelopes[pEditor->m_SelectedEnvelope];
		pNewEnv = NULL;
	...
	static int s_CreateCode = 0;
	if (pEditor->DoButton_Editor(&s_CreateCode, "Generate Code", 0, &Button, 0, 
		pEditor->m_Map.m_lEnvelopes[pEditor->m_SelectedEnvelope]->m_aName[0]?"Creating code for generate in appdata folder":"Envelope needs a name to be created"))
	{
		pEditor->EnvelopeGenerateCode(pEditor->m_Map.m_lEnvelopes[pEditor->m_SelectedEnvelope]);
		Close = 1;
	}
