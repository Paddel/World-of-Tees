...
void CLayerQuads::BrushPlace(CLayer *pBrush, float wx, float wy)
{
	...
	for(int i = 0; i < l->m_lQuads.size(); i++)
	{
		...
		for(int p = 0; p < 5; p++)
		{
			...
			if(m_pEditor->m_GridActive);
			{
				if(p == 4)
				{
					int LineDistance = m_pEditor->GetLineDistance();

					int GridFactor = m_pEditor->m_GridFactor;
					float qx = fx2f(l->m_lQuads[i].m_aPoints[p].x)+wx;
					float qy = fx2f(l->m_lQuads[i].m_aPoints[p].y)+wy;
					float x = 0.0f;
					float y = 0.0f;
					if(wx >= 0)
						x = (int)((qx+(LineDistance/2)*GridFactor)/(LineDistance*GridFactor)) * (LineDistance*GridFactor);
					else
						x = (int)((qx-(LineDistance/2)*GridFactor)/(LineDistance*GridFactor)) * (LineDistance*GridFactor);
					if(wy >= 0)
						y = (int)((qy+(LineDistance/2)*GridFactor)/(LineDistance*GridFactor)) * (LineDistance*GridFactor);
					else
						y = (int)((qy-(LineDistance/2)*GridFactor)/(LineDistance*GridFactor)) * (LineDistance*GridFactor);
					...
					for(int m = 0; m < 4; m++)
					{
						n.m_aPoints[m].x = f2fx(x) + n.m_aPoints[m].x - n.m_aPoints[4].x;
						n.m_aPoints[m].y = f2fx(y) + n.m_aPoints[m].y - n.m_aPoints[4].y;
					}

					n.m_aPoints[p].x = f2fx(x);
					n.m_aPoints[p].y = f2fx(y);
				}

			}
		}

		if(m_pEditor->m_RandomAnimateColor && n.m_ColorEnv != -1)
		{
			int Size = m_pEditor->m_Map.m_lEnvelopes[n.m_ColorEnv]->m_lPoints.size();
			int MaxTime = m_pEditor->m_Map.m_lEnvelopes[n.m_ColorEnv]->m_lPoints[Size-1].m_Time;
			n.m_ColorEnvOffset = rand()%MaxTime;
		}

		if(m_pEditor->m_RandomAnimatePosition && n.m_PosEnv != -1)
		{
			int Size = m_pEditor->m_Map.m_lEnvelopes[n.m_PosEnv]->m_lPoints.size();
			int MaxTime = m_pEditor->m_Map.m_lEnvelopes[n.m_PosEnv]->m_lPoints[Size-1].m_Time;
			n.m_PosEnvOffset = rand()%MaxTime;
		}

		if(m_pEditor->m_RandomAnimateSync && m_pEditor->m_RandomAnimateColor && m_pEditor->m_RandomAnimatePosition &&
			n.m_ColorEnv != -1 && n.m_PosEnv != -1)
		{
			int SizeCol = m_pEditor->m_Map.m_lEnvelopes[n.m_ColorEnv]->m_lPoints.size();
			int MaxTimeCol = m_pEditor->m_Map.m_lEnvelopes[n.m_ColorEnv]->m_lPoints[SizeCol-1].m_Time;
			int SizePos = m_pEditor->m_Map.m_lEnvelopes[n.m_PosEnv]->m_lPoints.size();
			int MaxTimePos = m_pEditor->m_Map.m_lEnvelopes[n.m_PosEnv]->m_lPoints[SizePos-1].m_Time;

			if(MaxTimeCol == MaxTimePos)
			{
				n.m_ColorEnvOffset = n.m_PosEnvOffset;
			}
		}
