...
int CEditor::ms_ExEntitiesTexture;
...
void CLayerGroup::DeleteLayer(int Index)
{
	...
	if(m_lLayers[Index] == m_pMap->m_pExLayer)
		m_pMap->m_pExLayer = 0;
...
float CEditor::UiDoValueSelectorF(void *pID, CUIRect *pRect, const char *pLabel, float Current, float Min, float Max, float Step, float Scale, const char *pToolTip, bool Float)
{
	// logic
	...
	char aBuf[128];
	if(Float)
		str_format(aBuf, sizeof(aBuf),"%s %.2f", pLabel, Current);
	else
		str_format(aBuf, sizeof(aBuf),"%s %.0f", pLabel, Current);
...
{
	return (float)UiDoValueSelectorF(pID, pRect, pLabel, (float)Current, (float)Min, (float)Max, (float)Step, Scale, pToolTip, false);
}
...
void CEditor::DoToolbar(CUIRect ToolBar)
{
	...
	TB_Bottom.VSplitLeft(70.0f, &Button, &TB_Bottom);
	float NewVal = UiDoValueSelectorF(&m_AnimateTime, &Button, "", m_AnimateTime, -999999, 999999, 0.1f, 2, "Editor time for animations.", true);
	m_AnimateStart += float(m_AnimateTime-NewVal)*1000000.0f;

	TB_Bottom.VSplitLeft(5.0f, 0, &TB_Bottom);
	TB_Bottom.VSplitLeft(40.0f, &Button, &TB_Bottom);

	char Buf[32];
	int t = ((int)m_AnimateTime%1440) + (m_AnimateTime >= 0.0? 0: 1439);
	str_format(Buf, sizeof(Buf), "%d:%02d", t/60, t%60);
	UI()->DoLabel(&Button, Buf, 10.0f, -1, -1);
	...
	static int s_RandAnimColor = 0;
	if(DoButton_Ex(&s_RandAnimColor, "Rand C", pQLayer?m_RandomAnimateColor:-1, &Button, 0, "Random Color start time.", CUI::CORNER_L))
	{
		m_RandomAnimateColor = !m_RandomAnimateColor;
	}

	TB_Bottom.VSplitLeft(45.0f, &Button, &TB_Bottom);
	static int s_RandAnimSync = 0;
	if(DoButton_Ex(&s_RandAnimSync, "Sync", (m_RandomAnimatePosition&m_RandomAnimateColor)?m_RandomAnimateSync:-1, &Button, 0, "Synchronize Position and Color time (only works if they have the same max-time).", 0))
	{
		m_RandomAnimateSync = !m_RandomAnimateSync;
	}

	TB_Bottom.VSplitLeft(45.0f, &Button, &TB_Bottom);
	static int s_RandAnimPosition = 0;
	if(DoButton_Ex(&s_RandAnimPosition, "Rand P", pQLayer?m_RandomAnimatePosition:-1, &Button, 0, "Random Position start time.", CUI::CORNER_R))
	{
		m_RandomAnimatePosition = !m_RandomAnimatePosition;
	}

	TB_Bottom.VSplitLeft(5.0f, 0, &TB_Bottom);
	TB_Bottom.VSplitLeft(55.0f, &Button, &TB_Bottom);
	static int s_ExMapInfo = 0;
	if(DoButton_Ex(&s_ExMapInfo, "Ex Mapinfo", m_UseExMapInfo, &Button, 0, "Extended Mapinfo", CUI::CORNER_ALL))
	{
		m_UseExMapInfo = !m_UseExMapInfo;
	}
}
...
void CEditor::DoMapEditor(CUIRect View, CUIRect ToolBar)
{
	...
	if(!m_ShowPicker)
	{
		...
		if(m_Map.m_pExLayer && m_Map.m_pGameGroup->m_Visible && m_Map.m_pExLayer->m_Visible)
		{
			m_Map.m_pExLayer->Render();
		}
		...
		if(UI()->HotItem() == s_pEditorID)
		{
			...
			else
			{
				...
				if(UI()->MouseButton(0) && s_Operation == OP_NONE)
				{
					...
					if((Input()->KeyPressed(KEY_LSHIFT) || Input()->KeyPressed(KEY_RSHIFT) || Input()->KeyPressed('d')) && pLayer && !m_ShowPicker)
...
void CEditor::CheckExTile(int x, int y)
{
	if(GetSelectedLayer(0) != m_Map.m_pExLayer)
		return;

	int Tile = m_Map.m_pExLayer->GetTile(x/32, y/32);
	if(Tile == -1)//outa map
		return;

	int Index = m_Map.m_pExLayer->GetIndex(Tile);

	int ExtInpID = -1;
	for(int i = 0; i < m_lpExtendInput.size(); i++)
	{
		if(m_lpExtendInput[i]->m_Index == Index)
		{
			ExtInpID = i;
			break;
		}
	...
	m_Map.m_pEditingExtInp = m_lpExtendInput[ExtInpID];
	m_Map.m_pEditingExTile = &m_Map.m_pExLayer->m_pExTiles[Tile];

	char *pArgs = m_Map.m_pEditingExTile->m_ExArgs;
	
	{
		...
		if(!Err)
		{
			for(int i = 0; pArgs; i++)
			{
				if(i >= m_Map.m_pEditingExtInp->m_lpInputArgs.size())
				{
					Err = true;
					break;
				}

				char *pIndicator = GetSepStr(0xff, &pArgs);
				if(pIndicator[0] == 'i')
				{
					if(m_Map.m_pEditingExtInp->m_lpInputArgs[i]->m_Type != EXTINPTYPE_INT)
					{
						Err = true;
						break;
					}

					int *pInpVal = &((CExtendInputInt *)m_Map.m_pEditingExtInp->m_lpInputArgs[i])->m_Val;
					*pInpVal = GetSepInt(0xff, &pArgs);
				}
				else if(pIndicator[0] == 's')
				{
					if(m_Map.m_pEditingExtInp->m_lpInputArgs[i]->m_Type != EXTINPTYPE_STR)
					{
						Err = true;
						break;
					}

					str_copy(((CExtendInputStr *)m_Map.m_pEditingExtInp->m_lpInputArgs[i])->m_aVal, GetSepStr(0xff, &pArgs), EXPINP_STR_MAX_LEN);
				}
			}
		}

		if(Err)
			for(int i = 0; i < m_Map.m_pEditingExtInp->m_lpInputArgs.size(); i++)
				m_Map.m_pEditingExtInp->m_lpInputArgs[i]->Reset();
	}

	static int s_ExtInpPopupID = 0;
	UiInvokePopupMenu(&s_ExtInpPopupID, 0, UI()->MouseX(), UI()->MouseY(), 200, 32+17*m_Map.m_pEditingExtInp->m_lpInputArgs.size()+12, PopupExtInp);
}
		...
		for(int g = 0; g < m_Map.m_lGroups.size(); g++)
		{
			...
			if(LayerCur >= LayerStartAt)
			{
					...
					BUTTON_CONTEXT, m_Map.m_lGroups[g]->m_Collapse ? "Select group. Double click to expand." : "Select group. Double click to collapse.", 0, FontSize))
				{
					...
					if(Result == 2)
						UiInvokePopupMenu(&s_GroupPopupId, 0, UI()->MouseX(), UI()->MouseY(), 145, 250, PopupGroup);
...
void CEditor::AddImage(const char *pFileName, int StorageType, void *pUser)
{
	...
	*pImg = ImgInfo;
	pImg->m_TexID = pEditor->Graphics()->LoadTextureRaw(ImgInfo.m_Width, ImgInfo.m_Height, ImgInfo.m_Format, ImgInfo.m_pData, CImageInfo::FORMAT_AUTO, 0);
	ImgInfo.m_pData = 0;
	pImg->m_External = !pEditor->m_ExtendedDir;	// external by default
...
int CEditor::PopupImage(CEditor *pEditor, CUIRect View)
{
	...
	{
		pEditor->InvokeFileDialog(IStorage::TYPE_ALL, FILETYPE_IMG, "Replace Image", "Replace", pEditor->m_ExtendedDir?"Graphics":"mapres", "", ReplaceImage, pEditor);
		return 1;
	}
...
void CEditor::RenderImages(CUIRect ToolBox, CUIRect ToolBar, CUIRect View)
{
	...
	for(int e = 0; e < 2; e++) // two passes, first embedded, then external
	{
		...
		for(int i = 0; i < m_Map.m_lImages.size(); i++)
		{
				...
				BUTTON_CONTEXT, "Select image"))
			{
				...
				if(Result == 2)
					UiInvokePopupMenu(&s_PopupImageID, 0, UI()->MouseX(), UI()->MouseY(), 120, 100, PopupImage);
			}
	...
	// new image
	static int s_NewImageButton = -1;
	ToolBox.HSplitTop(12.0f, &Slot, &ToolBox);
	if(DoButton_Editor(&s_NewImageButton, "Add", 0, &Slot, 0, "Load a new image to use in the map"))
		InvokeFileDialog(IStorage::TYPE_ALL, FILETYPE_IMG, "Add Image", "Add", m_ExtendedDir?"Graphics":"mapres", "", AddImage, this);

	ToolBox.HSplitTop(5.0f, &Slot, &ToolBox);

	static int s_ToggleDir = 0;
	ToolBox.HSplitTop(12.0f, &Slot, &ToolBox);
	if(DoButton_Editor(&s_ToggleDir, m_ExtendedDir?"E/D":"N/D", 0, &Slot, 0, "normal directory/ extended directory"))
		m_ExtendedDir = !m_ExtendedDir;
}
...
void CEditor::RenderFileDialog()
{
	...
	if(m_FileDialogStorageType == IStorage::TYPE_SAVE)
	{
		...
		if(DoButton_Editor(&s_MapInfoButton, "Map details", 0, &Button, 0, 0))
		{
			...
			UiInvokePopupMenu(&s_MapInfoPopupID, 0, Width/2.0f-200.0f, Height/2.0f-100.0f, 400.0f, 270.0f, PopupMapInfo);
			UI()->SetActiveItem(0);
		}
	}
}
...
void CEditor::RenderStatusbar(CUIRect View)
{
	...
	View.VSplitRight(60.0f, &View, &Button);
	static int s_MapInfo = 0;
	if(DoButton_Editor(&s_MapInfo, "MapInfo", m_UiNumPopups? -1 : 0, &Button, 0, "Edit the mapinfo.") && m_UiNumPopups == 0)
	{
		float Width = UI()->Screen()->w, Height = UI()->Screen()->h;
	...
	View.VSplitRight(5.0f, &View, &Button);
	View.VSplitRight(70.0f, &View, &Button);
	static int s_ChestContent = 0;
	if(DoButton_Editor(&s_ChestContent, "Chest content", m_UiNumPopups? -1 : 0, &Button, 0, "View the map content.") && m_UiNumPopups == 0)
	{
		float Width = UI()->Screen()->w, Height = UI()->Screen()->h;

		static int s_ChestContentPopupID = 0;
		UiInvokePopupMenu(&s_ChestContentPopupID, 0, Width/2.0f-200.0f, Height/2.0f-100.0f, 400.0f, 200.0f, PopupChestContent);
		UI()->SetActiveItem(0);
	}
...
void CEditor::EnvelopeGenerateCodePoint(IOHANDLE File, CEnvelope *pEnvelope, int Index)
{
	char aLine[512];
	CEnvPoint Point = pEnvelope->m_lPoints[Index];

	str_format(aLine, sizeof(aLine), "	{//Point #%i", Index);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_copy(aLine, "		CEnvPoint Point;", sizeof(aLine));
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		Point.m_Time = %i;", Point.m_Time);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		Point.m_Curvetype = %i;", Point.m_Curvetype);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		Point.m_aValues[0] = %i;", Point.m_aValues[0]);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		Point.m_aValues[1] = %i;", Point.m_aValues[1]);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		Point.m_aValues[2] = %i;", Point.m_aValues[2]);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		Point.m_aValues[3] = %i;", Point.m_aValues[3]);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "		p%s->m_lPoints.add(Point);", pEnvelope->m_aName);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_copy(aLine, "	}", sizeof(aLine));
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);
	io_write_newline(File);
}

void CEditor::EnvelopeGenerateCode(CEnvelope *pEnvelope)
{
	char aFileName[64];
	char aLine[512];
	IOHANDLE File = NULL;
	str_format(aFileName, sizeof(aFileName), "env_%s.cpp", pEnvelope->m_aName);
	File = Storage()->OpenFile(aFileName, IOFLAG_WRITE, IStorage::TYPE_SAVE);
	if (!File)
		return;

	str_format(aLine, sizeof(aLine), "void %s(CGenerator *pGenerator)", pEnvelope->m_aName);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);
	...
	str_format(aLine, sizeof(aLine), "	CGenEnvelope *p%s = new CGenEnvelope(%i);", pEnvelope->m_aName, pEnvelope->m_Channels);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);

	str_format(aLine, sizeof(aLine), "	p%s->m_Synchronized = %s;", pEnvelope->m_aName, pEnvelope->m_Synchronized?"true":"false");
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);
	io_write_newline(File);

	for (int i = 0; i < pEnvelope->m_lPoints.size(); i++)
		EnvelopeGenerateCodePoint(File, pEnvelope, i);

	str_format(aLine, sizeof(aLine), "	pGenerator->AddEnvelopeTemplate(p%s, \"%s\");", pEnvelope->m_aName, pEnvelope->m_aName);
	io_write(File, aLine, str_length(aLine));
	io_write_newline(File);
...
void CEditor::RenderEnvelopeEditor(CUIRect View)
{
	...
	bool IsTimeEnv = false;
	if(pEnvelope)
	{
		int MaxTime = pEnvelope->m_lPoints[pEnvelope->m_lPoints.size()-1].m_Time;
		if(MaxTime == 1440*1000 && pEnvelope->m_Synchronized)
			IsTimeEnv = true;
	}

	CUIRect ToolBar, CurveBar, ColorBar;
	View.HSplitTop(15.0f, &ToolBar, &View);
	View.HSplitTop(25.0f, &CurveBar, &View);
	...
	{
		CUIRect TimeLine;
		CurveBar.VSplitLeft(-10.0f, &TimeLine, NULL);
		TimeLine.y+=12;
		char aBuf[6];
		for(int i = 0; i < 24; i++)
		{
			TimeLine.x = i*(ViewWidth/24.0f)+ViewX;
			str_format(aBuf, sizeof(aBuf), "%i:00", i);
			UI()->DoLabel(&TimeLine, aBuf, 7.0f, 0, 0);
		}
	}
	...
	// do the toolbar
	{
		...
		if(m_SelectedEnvelope >= 0)
		{
			...
			static int s_ExtendedMenu = 0;
			if (DoButton_Editor(&s_ExtendedMenu, "^", 0, &Button, 0, "Extended envelope menu"))
			{
				static int s_EnvelopePopupID = 0;
				UiInvokePopupMenu(&s_EnvelopePopupID, 0, Button.x, Button.y-16, 90, 6 * 4 + 6 * 15, PopupEnvelopeMenu);
			}
		}
		else
		{
		...
		ToolBar.VSplitRight(50.0f, &ToolBar, &Button);
		m_AnimatePointSet = UiDoValueSelectorF(&m_AnimatePointSet, &Button, "", m_AnimatePointSet, -999999, 999999, 1, 2, "'c'=this time 'd'=down 'f'=up 'x'=last point 'g'=real time");
	...
	if(pEnvelope)
	{
		...
		// render lines
		{
			...
			if(pEnvelope->m_lPoints.size() >= 2)
			{
				Graphics()->SetColor(0.25f, 0.25f, 0.25f, 0.7f);
				int MaxTime = pEnvelope->m_lPoints[pEnvelope->m_lPoints.size()-1].m_Time;
				float t = ((int)(m_AnimateTime*1000)%MaxTime)/(float)MaxTime;
				IGraphics::CLineItem LineItem(View.x+View.w*t, View.y, View.x+View.w*t, View.y+View.h);
				Graphics()->LinesDraw(&LineItem, 1);
			}
					...
					if(UI()->ActiveItem() == pID)
					{
						...
						else
						{
							...
							else
							{
								...
								else if(Input()->KeyPressed('c'))
								{
									pEnvelope->m_lPoints[i].m_aValues[c] = f2fx(m_AnimatePointSet)/(m_Map.m_lEnvelopes[m_SelectedEnvelope]->m_Channels == 4?1000.0f:1);
								}
								else if(Input()->KeyPressed('g'))
								{
									if(i == pEnvelope->m_lPoints.size()-1)
										pEnvelope->m_lPoints[i].m_Time = 1440000;
								}
								else if(Input()->KeyPressed('f'))
								{
									pEnvelope->m_lPoints[i].m_aValues[c] = 1024;
								}
...
int CEditor::PopupMenuFile(CEditor *pEditor, CUIRect View)
{
	...
	View.HSplitTop(2.0f, &Slot, &View);
	View.HSplitTop(12.0f, &Slot, &View);
	if (pEditor->DoButton_MenuItem(&s_GenerateButton, "Generate", 0, &Slot, 0, "Generate a new map on this one"))
	{
		char *pGenPrevName = "OldMap";
		if (pEditor->m_aFileName[0] && pEditor->m_ValidSaveFilename)
...
void CEditorMap::MakeExLayer(CLayer *pLayer)
{
	m_pExLayer = (CLayerExTiles *)pLayer;
	m_pExLayer->m_pEditor = m_pEditor;
	m_pExLayer->m_TexID = m_pEditor->ms_ExEntitiesTexture;
}
...
void CEditor::Init()
{
	...
	ms_ExEntitiesTexture = Graphics()->LoadTexture("editor/exentities.png", IStorage::TYPE_ALL, CImageInfo::FORMAT_AUTO, 0);
	...
	m_Generator.SetStorage(m_pStorage);

	Reset();
	m_Map.m_Modified = false;
	
	RegisterExtendetTiles();
}
...
void CEditor::RegisterExtendetTiles()
{
	...
	RegisterExtendInput(EXTILE_DOOR_TRIGGER, "Door Trigger", "i;Door-ID;0;i;Visible;1");
	RegisterExtendInput(EXTILE_PUZZLE, "Puzzle", "s;Type;Pyramide;i;ID;0");
	RegisterExtendInput(EXTILE_NPC_TICKETSELLER, "Ticket-Seller", "i;Ticket;1");
	RegisterExtendInput(EXTILE_BOLDER, "Bolder", "i;ID;0;i;Damage;50");
	RegisterExtendInput(EXTILE_BOLDER_TRIGGER, "Bolder-Trigger", "i;ID;0");
	RegisterExtendInput(EXTILE_BOSS_SPAWNER, "Boss-Spawner", "s;Bossname;Pharao;i;Door-ID;0");
	RegisterExtendInput(EXTILE_WEAPON_MOUNT, "Weapon-Mount", "s;Weapon;Gun;i;Damage;1");
	RegisterExtendInput(EXTILE_GENMAP, "Generate Map", "i;ID;0;i;TransitionID;0;i;HammerNeeded;0");
}

void CEditor::RegisterExtendInput(int Index, char *pName, char *pArgs)
{
	for(int i = 0; i < m_lpExtendInput.size(); i++)
		if(m_lpExtendInput[i]->m_Index == Index)
			return;

	CExtendInput *pExtInp = new CExtendInput(Index, pName);
	char aArgBuf[1024];
	str_copy(aArgBuf, pArgs, sizeof(aArgBuf));
	pArgs = aArgBuf;
	//add argumentlist
	while(pArgs)
	{
		char mIndicator[3];
		str_copy(mIndicator, GetSepStr(';', &pArgs), sizeof(mIndicator));
		char *pName = GetSepStr(';', &pArgs);
		if(mIndicator[0] == 'i')
			pExtInp->m_lpInputArgs.add( new CExtendInputInt(pName, GetSepInt(';', &pArgs)) );
		else if(mIndicator[0] == 's')
		{
			pExtInp->m_lpInputArgs.add( new CExtendInputStr(pName, GetSepStr(';', &pArgs)) );
		}
		else
		{
			GetSepStr(0xff, &pArgs);
			GetSepStr(0xff, &pArgs);
		}
	}

	m_lpExtendInput.add(pExtInp);
}
