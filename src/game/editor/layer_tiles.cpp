...
void CLayerTiles::BrushFlipX()
{
	...
	if(m_LayerType == LAYERTILETYPE_NONE)
		for(int y = 0; y < m_Height; y++)
			for(int x = 0; x < m_Width; x++)
...
int CLayerTiles::RenderProperties(CUIRect *pToolBox)
{
	...
	if(m_LayerType != LAYERTILETYPE_NONE) // remove the image and color properties if this is the game layer
	{
		aProps[3].m_pName = 0;
		aProps[4].m_pName = 0;
	}
...
int CLayerTiles::GetTile(int x, int y)
{
...
int CLayerTiles::GetIndex(int Tile)
{
