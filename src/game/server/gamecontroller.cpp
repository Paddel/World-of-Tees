...
void CGameController::EvaluateSpawnType(CSpawnEval *pEval, int Type, CMap *pMap)
{
	// get spawn point
	for(int i = 0; i < m_aNumSpawnPoints[Type]; i++)
	{
		if(m_aaSpawnPoints[Type][i].m_pMap != pMap)
			continue;

		// check if the position is occupado
		CCharacter *aEnts[MAX_CLIENTS];
		int Num = GameServer()->m_World.FindEntities(m_aaSpawnPoints[Type][i].m_Pos, 64, (CEntity**)aEnts, MAX_CLIENTS, CGameWorld::ENTTYPE_CHARACTER, pMap);
		...
		for(int Index = 0; Index < 5 && Result == -1; ++Index)
		{
			Result = Index;
			for(int c = 0; c < Num; ++c)
				if(pMap->Collision()->CheckPoint(m_aaSpawnPoints[Type][i].m_Pos+Positions[Index]) ||
					distance(aEnts[c]->m_Pos, m_aaSpawnPoints[Type][i].m_Pos+Positions[Index]) <= aEnts[c]->m_ProximityRadius)
				{
					Result = -1;
					break;
				}
		...
		vec2 P = m_aaSpawnPoints[Type][i].m_Pos+Positions[Result];
...
bool CGameController::CanSpawn(int Team, vec2 *pOutPos, CMap *pMap)
{
	CSpawnEval Eval;
	...
	if(IsTeamplay())
	{
		...
		EvaluateSpawnType(&Eval, 1+(Team&1), pMap);
		if(!Eval.m_Got)
		{
			EvaluateSpawnType(&Eval, 0, pMap);
			if(!Eval.m_Got)
				EvaluateSpawnType(&Eval, 1+((Team+1)&1), pMap);
		}
	}
	else
	{
		EvaluateSpawnType(&Eval, 0, pMap);
		EvaluateSpawnType(&Eval, 1, pMap);
		EvaluateSpawnType(&Eval, 2, pMap);
	}
...
bool CGameController::OnEntity(int Index, vec2 Pos, CMap *pMap)
{
	if(Index == ENTITY_SPAWN)
	{
		m_aaSpawnPoints[0][m_aNumSpawnPoints[0]].m_Pos = Pos;
		m_aaSpawnPoints[0][m_aNumSpawnPoints[0]].m_pMap = pMap;
		m_aNumSpawnPoints[0]++;
	}
	else if(Index == ENTITY_SPAWN_RED)
	{
		m_aaSpawnPoints[1][m_aNumSpawnPoints[1]].m_Pos = Pos;
		m_aaSpawnPoints[1][m_aNumSpawnPoints[1]].m_pMap = pMap;
		m_aNumSpawnPoints[1]++;
	}
	else if(Index == ENTITY_SPAWN_BLUE)
	{
		m_aaSpawnPoints[2][m_aNumSpawnPoints[2]].m_Pos = Pos;
		m_aaSpawnPoints[2][m_aNumSpawnPoints[2]].m_pMap = pMap;
		m_aNumSpawnPoints[2]++;
	}
	return false;
}
...
void CGameController::OnCharacterSpawn(class CCharacter *pChr)
{
	...
	//pChr->GiveWeapon( new CHammer(GameServer(), pChr, 25) );
	/*pChr->GiveWeapon( new CShotgun(GameServer(), pChr, 10) );
	pChr->GiveWeapon( new CRifle(GameServer(), pChr, 80) );
	pChr->GiveWeapon( new CGun(GameServer(), pChr, 10) );
	pChr->GiveWeapon( new CGrenade(GameServer(), pChr, 10) );
	pChr->GiveWeapon( new CDagger(GameServer(), pChr, 10) );*/
...
void CGameController::Snap(int SnappingClient)
{
	...
	pGameInfoObj->m_RoundStartTick = Server()->Tick()-GameServer()->GetIngameTime();
