...
CGenAutoMapper::CGenAutoMapper(IStorage *pStorage)
{
	m_pStorage = pStorage;
	m_FileLoaded = false;
}

void CGenAutoMapper::Load(const char* pTileName)
{
	char aPath[256];
			...
			else
			{
				...
				else if(!str_comp_num(pLine, "Pos", 3) && pCurrentIndex)
				{
					...
					if(!str_comp(aValue, "FULL"))
						Value = CPosRule::FULL;
					else if (!str_comp(aValue, "WATER"))
						Value = CPosRule::WATER;
	...
	dbg_msg("Automap","loaded %s", aPath);

	m_FileLoaded = true;
}

const char* CGenAutoMapper::GetConfigName(int Index)
{
...
void CGenAutoMapper::Proceed(CGenLayerTiles *pLayer, class CGenLayerTiles *pGameLayer, int ConfigID)
{
				...
				for (int j = 0; j < pConf->m_aIndexRules[i].m_aRules.size() && RespectRules; ++j)
				{
					...
					else
					{
						...
						else
						{
							...
							if (pGameLayer->m_pTiles[CheckIndex].m_Index != TILE_WATER && pRule->m_Value == CPosRule::WATER)
								RespectRules = false;
						}
					}
				}
...
void CGenAutoMapper::Proceed(CGenLayerTiles *pLayer, int ConfigID)
{
	if (!m_FileLoaded || pLayer->m_Readonly || ConfigID < 0 || ConfigID >= m_lConfigs.size())
		return;
